...
export const AdminGuard: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  if (showLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#040404] text-white" id="admin-guard-loading">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-white mx-auto mb-4" id="admin-guard-spinner"></div>
          <p className="text-lg font-semibold mb-2" id="admin-guard-loading-title">Checking your access…</p>
          <p className="text-sm text-white/60" id="admin-guard-loading-subtitle">Please wait while we verify your permissions</p>
        </div>
      </div>
    );
  }
  ...
  if (sessionValid === false || !isAuthenticated || !user) {
    if (!hasCheckedSession) {
      return (
        <div className="min-h-screen flex items-center justify-center bg-[#040404] text-white" id="admin-guard-session-check">
          <div className="text-center">
            <p className="text-lg font-semibold mb-2" id="admin-guard-session-check-title">Checking your access…</p>
            <p className="text-sm text-white/60" id="admin-guard-session-check-subtitle">Please wait while we verify your permissions</p>
          </div>
        </div>
      );
    }
  ...
  if (role === 'anonymous') {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#040404] text-white" id="admin-guard-role-loading">
        <div className="text-center">
          <p className="text-lg font-semibold mb-2" id="admin-guard-role-loading-title">Loading permissions…</p>
          <p className="text-sm text-white/60" id="admin-guard-role-loading-subtitle">Almost ready</p>
        </div>
      </div>
    );
  }
  ...
  if (role !== 'admin') {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-[#040404] text-white p-6" id="admin-guard-access-denied">
        <p className="text-center text-xl font-semibold" id="admin-guard-access-denied-title">Admin access required</p>
        <p className="max-w-sm text-center text-sm text-white/60" id="admin-guard-access-denied-message">
          You must sign in with an administrator account to manage content. Current role: <strong>{role}</strong>
        </p>
        <div className="flex gap-3" id="admin-guard-access-denied-buttons">
          <button
