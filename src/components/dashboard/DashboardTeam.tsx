    ...
    try {
      ...
      try {
        if (editingMember?.id) {
          ...
          // Create new member
          console.log('Adding new team member with data:', JSON.stringify({
            ...memberData,
            status: 'active',
          }, null, 2));
          await addTeamMember({
          ...
          >
            ...
            <div className="space-y-2 mb-4">
              <div className="flex items-center space-x-2 text-sm text-white/70">
                ...
                <a className="hover:text-[#E93370] transition-colors truncate" href={`mailto:${member.email}`}>
                  {member.email}
                </a>
              </div>
              {member.social_links?.instagram && (
                <div className="flex items-center space-x-2 text-sm text-white/70">
                  <svg aria-hidden="true" className="h-4 w-4 text-[#E93370] flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
