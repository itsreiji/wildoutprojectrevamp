-- Migration: 20250118_final_seed_from_b62b3e27
-- Description: Final consolidated seed data from commit b62b3e27dca9ccd6dd8fd58bdd6745b618e9d3f9
-- Includes: 4 events, 12 partners, 12 team members, 6 gallery items, hero/about/settings content
-- Created: 2025-01-18
...
-- Update partners category constraint to allow additional categories
DO $$ 
BEGIN
    -- Drop existing constraint if it exists
    IF EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'partners_category_check'
    ) THEN
        ALTER TABLE public.partners DROP CONSTRAINT partners_category_check;
    END IF;
    
    -- Add new constraint with additional categories
    ALTER TABLE public.partners ADD CONSTRAINT partners_category_check 
        CHECK (category IN ('venue', 'promoter', 'artist', 'sponsor', 'other', 'music', 'beverage', 'lifestyle', 'technology'));
END $$;

-- Update events category constraint to allow additional categories
DO $$ 
BEGIN
    -- Drop existing constraint if it exists
    IF EXISTS (
    ...
    ALTER TABLE public.events ADD CONSTRAINT events_category_check 
        CHECK (category IN ('music', 'sports', 'arts', 'food', 'community', 'other', 'festival', 'concert', 'exhibition', 'club', 'conference'));
END $$;

-- Update gallery_items relationship constraint to allow general category without relationships
DO $$ 
BEGIN
    -- Drop existing constraint if it exists
    IF EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'gallery_item_relationship_check'
    ) THEN
        ALTER TABLE public.gallery_items DROP CONSTRAINT gallery_item_relationship_check;
    END IF;
    
    -- Add new constraint that allows general category without relationships
    ALTER TABLE public.gallery_items ADD CONSTRAINT gallery_item_relationship_check 
        CHECK ((event_id IS NOT NULL) OR (partner_id IS NOT NULL) OR (category = 'general'));
END $$;

-- =============================================
-- CLEANUP EXISTING DATA (per user instruction)
-- =============================================

-- Delete non-published events
DELETE FROM public.events WHERE status != 'published';

-- Delete inactive partners
DELETE FROM public.partners WHERE status != 'active';

-- Delete inactive team members
DELETE FROM public.team_members WHERE status != 'active';

-- Delete non-published gallery items
DELETE FROM public.gallery_items WHERE status != 'published';

-- =============================================
-- SEED PARTNERS (12 partners from commit b62b3e27)
-- =============================================
...
-- =============================================
-- SEED TEAM MEMBERS (12 members from commit b62b3e27)
-- =============================================
...
-- =============================================
-- SEED EVENTS (4 events from commit b62b3e27)
-- =============================================
...
-- =============================================
-- SEED GALLERY ITEMS (6 items from commit b62b3e27)
-- =============================================
INSERT INTO public.gallery_items (id, title, description, image_url, category, status, event_id, tags, metadata, created_at, updated_at)
VALUES 
  ('00000000-0000-0000-0000-000000000051'::uuid, 'Neon Nights Festival', 'Neon Nights Festival', 'https://images.unsplash.com/photo-1709131482554-53117b122a35?w=800', 'event', 'published', (SELECT id FROM public.events WHERE title = 'Neon Nights: Electronic Odyssey' LIMIT 1), ARRAY['festival', 'neon', 'electronic']::text[], '{"event": "Neon Nights"}'::jsonb, now(), now()),
  ('00000000-0000-0000-0000-000000000052'::uuid, 'Concert Crowd', 'Concert Crowd', 'https://images.unsplash.com/photo-1656283384093-1e227e621fad?w=800', 'event', 'published', (SELECT id FROM public.events WHERE title = 'Sunset Sessions Vol. 12' LIMIT 1), ARRAY['concert', 'crowd', 'music']::text[], '{"event": "Sunset Sessions"}'::jsonb, now(), now()),
  ('00000000-0000-0000-0000-000000000053'::uuid, 'DJ Performance', 'DJ Performance', 'https://images.unsplash.com/photo-1761637955469-ee3c0dfdfb34?w=800', 'event', 'published', (SELECT id FROM public.events WHERE title = 'Bass Rebellion: Underground Series' LIMIT 1), ARRAY['dj', 'performance', 'bass']::text[], '{"event": "Bass Rebellion"}'::jsonb, now(), now()),
  ('00000000-0000-0000-0000-000000000054'::uuid, 'Art Exhibition', 'Art Exhibition', 'https://images.unsplash.com/photo-1599949287142-9a208b301ecd?w=800', 'event', 'published', (SELECT id FROM public.events WHERE title = 'Urban Art Exhibition' LIMIT 1), ARRAY['art', 'exhibition', 'urban']::text[], '{"event": "Urban Art"}'::jsonb, now(), now()),
  ('00000000-0000-0000-0000-000000000055'::uuid, 'Creative Team', 'Creative Team', 'https://images.unsplash.com/photo-1676277757211-ebd7fdeb3d5b?w=800', 'general', 'published', NULL, ARRAY['team', 'creative']::text[], '{}'::jsonb, now(), now()),
  ('00000000-0000-0000-0000-000000000056'::uuid, 'Brand Partnership', 'Brand Partnership', 'https://images.unsplash.com/photo-1758922801699-09d8d788f90c?w=800', 'general', 'published', NULL, ARRAY['partnership', 'brand']::text[], '{}'::jsonb, now(), now())
ON CONFLICT (id) DO UPDATE SET 
  title = EXCLUDED.title,
  description = EXCLUDED.description,
  image_url = EXCLUDED.image_url,
...
-- =============================================
-- SEED HERO CONTENT (from commit b62b3e27)
-- Singleton table - delete existing, then insert
-- =============================================
DELETE FROM public.hero_content;
INSERT INTO public.hero_content (id, title, subtitle, description, stats, cta_text, cta_link, created_at, updated_at)
VALUES (
...
-- =============================================
-- SEED ABOUT CONTENT (from commit b62b3e27)
-- Singleton table - delete existing, then insert
-- =============================================
DELETE FROM public.about_content;
INSERT INTO public.about_content (id, title, subtitle, founded_year, story, features, created_at, updated_at)
VALUES (
...
-- =============================================
-- SEED SITE SETTINGS (from commit b62b3e27)
-- Singleton table - delete existing, then insert
-- =============================================
DELETE FROM public.site_settings;
INSERT INTO public.site_settings (id, site_name, site_description, tagline, email, phone, address, social_media, created_at, updated_at)
VALUES (
